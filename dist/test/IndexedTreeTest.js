...
describe("IndexedTree", function () {
    ...
    it("Tree.fromNodeList returns correct tree in case with single root", function () {
        ...
        let tree = IndexedTree_1.IndexedTree.fromNodeList(nodeList, { id: "uid", data: (el) => ({ category: el.category }) });
        tree.children.get(0).children.get(1).data.should.deep.equal({ category: "drama" });
        tree = IndexedTree_1.IndexedTree.fromNodeList(nodeList, { id: "uid", data: (el) => ({ category: el.category }) }, true);
        tree.virtual.should.be.true;
        tree.children.get(0).id.should.equal("-");
        tree._count.should.equal(6);
    });
    it("Count returns the correct number of nodes", function () {
        const tree = this.tree;
        tree._count.should.equal(6);
    });
    it("Indexer can be set and read", function () {
        const tree = new IndexedTree_1.IndexedTree("root");
        tree.contains("root").should.be.true;
        let indexer = (node) => `${node.id}foo`;
    ...
    it("Contains is true if node exists otherwise false", function () {
        ...
        let c1 = new IndexedTree_1.IndexedTree("c1");
    ...
    it("Add and AddTo with subtrees and reindexing", function () {
        const tree = this.tree;
        let subtree = new IndexedTree_1.IndexedTree("subtree", tree.indexer);
        subtree.add("c5", "c5");
        subtree.addTo("c5", "c6", "c6");
        subtree.contains("c6").should.be.true;
        let c7 = new IndexedTree_1.IndexedTree("c7", tree.indexer);
        ...
        let wIndex = new IndexedTree_1.IndexedTree("wIndex", tree.indexer, tree.index);
        tree.add(wIndex);
        wIndex.add(subtree);
    ...
    it("Cut returns a node without its parent", function () {
        const tree = this.tree.clone();
        let child = tree.children.get(1).cut();
