...
describe("Dom", function () {
    ...
    it("FindAll finds right elements", function () {
        const el = Dom.create(this.html2);
        this.document.body.appendChild(el);
    ...
    it("findParent finds parent matching selector or null", function () {
        const el = Dom.create(this.html2);
        this.document.body.appendChild(el);
        const chld = Dom.find("#sub1");
        ...
        chai_2.expect(non).to.equal(null);
        el.parentNode.removeChild(el);
    });
    it("Get gets elementById", function () {
        const el = Dom.create(this.html2);
        this.document.body.appendChild(el);
    ...
    it("Is matches element with selector", function () {
        const el = Dom.create(this.html2);
        Dom.is("#foo", el).should.be.true;
        Dom.is(".bar", el).should.be.false;
    });
    it("setStylesExplicitly reads styles (from classes) and sets them as explicit styles", function () {
        const el = Dom.create(this.html2);
        this.document.body.appendChild(el);
    ...
    it("OuterHTML returns correct html", function () {
        const el = Dom.create(this.html2);
        Dom.outerHTML(el).should.equal(this.html2);
    });
    it("Position sets style top and left", function () {
        const el = Dom.create(this.html2);
        Dom.position(el, 10, 20);
        el.style.left.should.equal("10px");
        el.style.top.should.equal("20px");
    });
    it("Remove removes the child from the parent", function () {
        const el = Dom.create(this.html2);
        this.document.body.appendChild(el);
    ...
    it("Replace replaces an element with another", function () {
