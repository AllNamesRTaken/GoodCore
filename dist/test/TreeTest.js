...
describe("Tree", function () {
    ...
    it("Aggregate does aggregate values", function () {
        const tree = this.tree;
        let result = tree.aggregate((cur, i, collected) => {
            return Test_1.isNull(collected) ? cur.data : [cur.data, ...collected].join(",");
        });
    ...
    it("Root returns root of tree", function () {
        const tree = this.tree;
        (tree.children.get(1).children.get(1).root === tree).should.be.true;
    });
    it("Nodes are initable", function () {
        const tree = new Tree_1.Tree("newNode");
        tree.init({ data: "dataString" });
        tree.data.should.equal("dataString");
    });
    it("ForEach loops over all nodes and passes index in children", function () {
        const tree = this.tree;
        let iList = [];
        let dList = [];
        tree.forEach((el, i) => {
    ...
    it("Find by size finds the correct node", function () {
        const tree = this.tree;
    ...
    it("getLeaf gets a leaf by position", function () {
        const tree = this.tree;
        tree.getLeaf(0).data.should.equal("c1");
        tree.getLeaf(2).data.should.equal("c2-2");
        Test_1.isNull(tree.getLeaf(4)).should.be.true;
        Test_1.isNull(tree.find(-1)).should.be.true;
    });
    it("Filter returns filtered tree", function () {
        ...
        Test_1.isNull(filtered.children.get(0).children).should.be.true;
    ...
    it("Remove sets inner children list to null", function () {
        const tree = this.tree.clone();
        const c2 = tree.find((node) => node.data === "c2");
        c2.children.get(0).remove();
        c2.children.get(0).remove();
        Test_1.isNull(c2.children).should.be.true;
    });
    it("leafCount is recalculated after Add", function () {
        const tree = this.tree.clone();
        const c2 = tree.find((node) => node.data === "c2");
        c2.add("c2-3");
        tree.leafCount.should.equal(5);
    });
    it("weight changes size but not leafCount", function () {
        const tree = this.tree.clone();
        const c2 = tree.find((node) => node.data === "c2");
        c2.weight = 10;
    ...
    it("InsertAt inserts at the correct position", function () {
        ...
        tree.insertAt(2, new Tree_1.Tree().init({ data: "c2.5" }));
    ...
    it("Prune removes all children from a node", function () {
        ...
        Test_1.isNull(tree.children.get(1).children).should.be.true;
