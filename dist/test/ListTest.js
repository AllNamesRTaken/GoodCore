...
describe("List", function () {
    ...
    it("Copy copies values correctly into target", function () {
        ...
        indexed._index.keys.should.deep.equal(["1", "2", "4", "7"]);
        indexed._index.values.should.deep.equal([1, 4, 7, 2]);
        indexed._index.get(7).should.equal(7);
    });
    ...
    it("trucate with negative size sets length to 0", function () {
        const list = this.list1.clone();
    ...
    it("ShallowCopy references same inner objects", function () {
        ...
        indexed._index.keys.should.deep.equal(["0", "1"]);
        indexed._index.values.should.deep.equal(list.values);
    });
    ...
    it("Append appends a lists values or an array to another Lists values", function () {
        ...
        indexed._index.keys.should.deep.equal(["1", "2", "4", "7"]);
        indexed._index.values.should.deep.equal([1, 2, 4, 7]);
    });
    ...
    it("Clear sets the size to 0", function () {
        ...
        list1._index.keys.should.deep.equal([]);
        list1._index.values.should.deep.equal([]);
    });
    it("Add pushes a value onto the List and returns the list", function () {
        ...
        list1._index.get(42).should.equal(42);
    });
    ...
    it("Pop removes the last element in the list and returns it", function () {
        const list1 = this.list1.clone();
        list1.indexer = (el) => el;
        list1._index.get(2).should.equal(2);
        list1.pop().should.equal(2);
        (list1._index.get(2) === undefined).should.be.true;
    });
    it("Shift removes the first element in the list and returns it", function () {
        const list1 = this.list1.clone();
        list1.indexer = (el) => el;
        list1._index.get(1).should.equal(1);
        list1.shift().should.equal(1);
        (list1._index.get(1) === undefined).should.be.true;
    });
    it("Contains checks if a list contains a certain element", function () {
        ...
        indexed._index.get(4).should.equal(4);
    });
    it("Remove removes an element from the list", function () {
        ...
        (list1._index.get(4) === undefined).should.be.true;
    });
    ...
    it("RemoveAt removes the element at a given position", function () {
        ...
        (list1._index.get(7) === undefined).should.be.true;
    });
    ...
    it("SelectInto uses supplied list", function () {
        ...
        list2.indexer = (el) => el;
        (list2._index.get(1) === undefined).should.be.true;
        (list2._index.get(7)).should.equal(7);
        list2.values.should.deep.equal([7, 2]);
    ...
    it("MapInto maps correctly and sets length", function () {
        ...
        (list2._index.values).should.deep.equal([1, 4, 7, 2]);
