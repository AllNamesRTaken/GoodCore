...
export function destroy(obj: Object): void {
  if (
    obj.constructor &&
    isFunction((obj.constructor.prototype as any).destroy)
  ) {
...
export function equals(a: any, b: any): boolean {
  ...
  if (a !== b && a instanceof Object && isSameClass(a as Object, b as Object)) {
    if (isArray(a)) {
      ...
      (a as Object).constructor &&
      isFunction(((a as Object).constructor.prototype as any).equals)
    ) {
      // Compare Comparables
      ...
      while (++i < len && result) {
        ...
        if (!result) {
          if (
            isFunction((a as Indexable<any>)[key]) &&
            isFunction((b as Indexable<any>)[key])
          ) {
            result = true;
          }
        }
...
export function shallowCopy<T extends object, K extends keyof T>(
  obj: T,
): { [P in K]: T[P] } {
...
export function clone<T>(obj: T): T {
  ...
  let isNullObject =
    ...
    !(obj as unknown as Object).constructor;
  if (!(obj instanceof Object) && !isNullObject) {
    result = obj;
  } else if (
    obj instanceof Object &&
    isFunction(((obj as unknown as Object).constructor.prototype as any).clone)
  ) {
    //Cloneable
...
export function mixin<T = Indexable<any>>(
  target: Indexable<any> = {},
  exclude: Indexable<any> | null,
  ...sources: Array<Indexable<any>>
): T {
  const result = target;
...
export function setProperties(
  ...
  mapping?: Indexable<string>,
  limitToExisting: boolean = false,
): void {
...
export function transform<T extends { [index: string]: any }, S = T, U = any>(
  ...
  accumulator =
    accumulator ||
    (isObject(target) ? (Object.create(target) as S) : ({} as S));
...
export function toLookup<T>(
  a: Array<T>,
  hashFn: (el: T) => string = defaultHashFunction,
): Indexable<boolean> {
  return a.reduce((p, c) => {
    p[hashFn(c)] = true;
    return p;
...
export function arrayDiff<T, S = T>(
  a: Array<T>,
  b: Array<S>,
...
export function difference<
  ...
  S extends Indexable<any> = T,
>(target: T, base: S): T {
  function changes<T extends Indexable<any>, S extends Indexable<any> = T>(
    target: T,
    base: S,
  ): T {
    return transform(target, function (result, value: any, key: string) {
      if (isDifferent(value, base[key])) {
        (result as Indexable<any>)[key] =
