...
class DomState {
  ...
  public static _document: Document | undefined = Global.window
    ? Global.window.document
    : undefined;
  public static _el: Element | undefined = Global.window
    ? Global.window.document.createElement("div")
    : undefined;
  public static _parser: DOMParser | undefined = Global.window
    ? new DOMParser()
    : undefined;
  public static _template: HTMLTemplateElement | undefined = Global.window
    ? Global.window.document.createElement("template")
    : undefined;
  public static _fragment: DocumentFragment | undefined = Global.window
    ? Global.window.document.createDocumentFragment()
    : undefined;
}
  ...
  if (/^[a-zA-Z]+$/.test(html)) {
    ...
    if (usesTemplate) {
      ...
      assert(
        ...
        "Dom.create was unable to parse html",
      );
...
export function resolve(
  ...
  let result = isString(target)
    ? root &&
      root !== (DomState._document as any) &&
      is(target as string, root!)
      ? root
...
export function remove(element: Element | Node): Element | Node | null {
  return isNullOrUndefined(element.parentNode)
    ? null
    : (element.parentNode!.removeChild(element) as Element | Node);
}
...
export function find<T extends HTMLElement>(
  selector: string,
  root?: Element,
): T | null {
...
export function children(root: Element, selector?: string): Element[] {
  ...
  return selector === undefined
    ? children
    : children.filter((el) => is(selector, el));
}
export function findParent<T extends HTMLElement>(
  start: Element,
...
export function is(selector: string | Element, element: Element): boolean {
  ...
  if (!selector || !element || (selector as any) === document) {
    ...
    assert(
      !(isElement(selector) || isString(element)),
      "Did you mix up the arguments for Dom::is() ?",
    );
...
export function findClosestCommonParent(
  ...
  if (elements.length === 1) {
    ...
    let parents = elements.map((el) => {
      ...
      assert(
        parents.length >= startAtIndex,
        "startAtIndex higher than element parent depth",
      );
      if (startAtIndex > 0) {
    ...
    if (minDepth > 0) {
      ...
      while (!found && i < minDepth) {
        found = !parents.reduce(
          (p, c) => p === true && c[i] === firstList[i],
          true,
        );
        ++i;
      }
