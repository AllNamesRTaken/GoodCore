...
export class Tree<T> implements ISerializable<T[]>, ICloneable, IInitable {
  ...
  protected _weight = 1;
  protected _listeners: Indexable<Indexable<
    (targets?: Array<Tree<T>>) => void
  > | null> = {};
  ...
  public static fromObject<T>(obj: Indexable<any>): Tree<T> {
    ...
    const root = new Tree<T>(obj.id as any).init({
      data: obj.data as any,
      parent,
      isDirty: true,
    });
    if (obj.children !== undefined && isArray(obj.children)) {
      root.children = (obj.children as Array<Tree<T>>).map(
  ...
  public on(
    event: TreeEvent,
    callback: (targets: Array<Tree<T>>) => void,
  ): number {
  ...
  public off(event: TreeEvent, index?: number): void {
    ...
    this._listeners[event] &&
      Object.keys(this._listeners[event]!).length === 0 &&
  ...
  public trigger(event: TreeEvent, targets: Array<Tree<T>>): void {
    if (this.isEventing) {
      switch (event) {
        case "change":
          this._listeners[event] &&
            forEach(this._listeners[event]!, (fn) => fn(targets));
  ...
  protected create<S = T>(id?: string | number): this {
    return new ((this as Tree<T>).constructor as Constructor<Tree<T>>)(
      id,
    ) as this;
  }
  ...
  public prune(): this {
    if (this.children !== null) {
      this.children.forEach(function (el: Tree<T>, i: number) {
        el.parent = null;
      });
        ...
        [cur._size, cur._leafCount] =
          isNull(agg) || agg!.length === 0
            ...
            : agg!.reduce(
                ...
                [
                  cur.weight,
                  cur.children === null || cur.children.length === 0 ? 1 : 0,
                ],
              );
        cur.isDirty = false;
  ...
  public filter(
    ...
    if (condition(this)) {
      ...
      node.parent = parent;
      if (children !== null) {
        node.children = children!.filter(condition).map(function (
          el: Tree<T>,
          i: number,
        ) {
  ...
  public toJSON() {
    ...
    this.forEach((node) => {
      result.push({
        ...
        children:
          node.children === null ? null : node.children.map((el) => el.id),
      });
    });
    return result;
  }
  public serialize(): T[] {
    ...
    result.push({
      ...
      children:
        this.children === null ? null : this.children.map((el) => el.id),
    });
    if (this.children !== null) {
