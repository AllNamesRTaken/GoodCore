  ...
  implements ISerializable<IObject>, IDeserializable<Dictionary<T>>, ICloneable
{
  ...
  constructor() {
    ...
    (this as any)["delete"] = (key: number | string) => {
      once(() => {
        console.warn(
          "Function Dictionary::delete(id) is deprecated please use Dictionary::remove instead. set is a reserved word.",
        );
      });
  ...
  public get(key: number | string): T | undefined {
    once(() => {
      console.warn(
        "Function Dictionary::get(id) is deprecated please use Dictionary::lookup instead. get is a reserved word.",
      );
    });
  ...
  public set(key: number | string, value: T): this {
    once(() => {
      console.warn(
        "Function Dictionary::set(id) is deprecated please use Dictionary::add instead. set is a reserved word.",
      );
    });
  ...
  public serialize(): IObject {
    ...
    forEach(this.keys, (key) => {
      ...
      obj[key] =
        isNotNullOrUndefined(v) && isFunction((v as any).serialize)
          ? (v as any).serialize()
          : v;
    });
    return obj;
    ...
    if (isNotUndefined(T)) {
      if (isNotUndefined(T!.prototype.deserialize)) {
        for (let i = 0; i < keys.length; i++) {
          ...
          this.add(
            key,
            t.deserialize.apply(t, [obj[key]].concat(passthroughT)),
          );
        }
      } else {
        for (let i = 0; i < keys.length; i++) {
