    ...
    ICloneable
{
    ...
    return {
      ...
      value:
        this._pointer < this.length
          ...
          : (((this._pointer = 0), undefined as any) as any),
    };
  }
  protected create<S = T>(arr?: S[] | List<S>): List<S> {
    return new ((this as unknown as object).constructor as Constructor<
      List<S>
    >)(arr);
  }
  public get values(): T[] {
  ...
  public get(pos: number): T | undefined {
    once(() => {
      console.warn(
        "Function List::get(id) is deprecated please use List::read instead. get is a reserved word.",
      );
    });
  ...
  public set(pos: number, v: T): this {
    once(() => {
      console.warn(
        "Function List::get(id) is deprecated please use List::write instead. get is a reserved word.",
      );
    });
  ...
  public write(pos: number, v: T): this {
    if (pos >= 0 && pos < this.length) {
      ...
      throw new Error(
        `index out of bounds on <List>.set(${pos}, ${(v as Object).toString()})`,
      );
    }
    return this;
  }
  ...
  public tail(count: number = 1): this {
    ...
    return this.create(
      this._array.slice(Math.max(0, this._array.length - count)),
    ) as this;
  }
  public splice(
    pos: number = 0,
    remove: number = Infinity,
    insert: T[] | this = [],
  ): this {
    this._array.splice(
      pos,
      remove,
      ...(isArray(insert) ? (insert as T[]) : (insert as this).values),
    );
    this._reindex();
    return this;
  }
  ...
  public zip<U, V>(
    list: List<U>,
    fn: (t: T, u: U) => V = (t: T, u: U) => [t, u] as any,
  ): List<V> {
  ...
  public unzip<U, V>(
    fn: (el: T) => [U, V] = (el: any) => [(el as [U, V])[0], (el as [U, V])[1]],
  ): [List<U>, List<V>] {
  ...
  public flatten<U>(maxDepth: number = Infinity): List<U> {
    return this.create<U>(
      maxDepth < 1
        ? (this.values as any)
        : this._flattenInner(this.values, maxDepth),
    );
  }
  private _flattenInner<U>(
    src: any[],
    maxDepth: number,
    depth: number = -1,
    result: U[] = [],
  ...
  public serialize(): T[] {
    return this.values.map((el) =>
      isFunction((el as any).serialize)
        ? ((el as unknown as ISerializable<any>).serialize() as T)
        : el,
    );
  }
