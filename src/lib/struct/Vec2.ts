...
export class Vec2 implements IVec2 {
  ...
  public set(src: IVec2): this {
    once(() => {
      console.warn(
        "Function Vec2::set is deprecated please use Vec2::copy instead. get is a reserved word.",
      );
    });
  ...
  public almostEquals(target: IVec2): boolean {
    return (
      Math.abs(this.x - target.x) < Vec2Const.EPSILON &&
      Math.abs(this.y - target.y) < Vec2Const.EPSILON
    );
  }
  ...
  public dot(vectorB: IVec2): number {
    return this.x * vectorB.x + this.y * vectorB.y;
  }

  public cross(vectorB: IVec2): number {
