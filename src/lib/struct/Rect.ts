import { IRect } from "../../@types/IRect.js";
...
export class Rect implements IRect {
  ...
  public set(src: IRect): this {
    once(() => {
      console.warn(
        "Function Rect::set is deprecated please use Rect::copy instead. get is a reserved word.",
      );
    });
  ...
  public clone(out?: this): this {
    const result = out
      ? out.copy(this)
      : this.create(this.start.x, this.start.y, this.stop.x, this.stop.y);
    return result;
  }
  public fromRange2(range: IRange2, endInclusive: boolean = false): this {
    ((this.start.x = range.pos.x),
      ...
      (this.stop.x =
        range.pos.x -
        (endInclusive ? (range.size.x < 0 ? -1 : 1) : 0) +
        range.size.x),
      (this.stop.y =
        range.pos.y -
        (endInclusive ? (range.size.y < 0 ? -1 : 1) : 0) +
        range.size.y),
      (this.endInclusive = endInclusive));
    return this;
  }
  ...
  public intersect(target: IRect): boolean {
    return (
      this.containsPoint(target.start.x, target.start.y) ||
