...
export default class Rect {
    ...
    public Clone(out?: Rect): Rect {
        let result = out ? out.Set(this) : new Rect(this.start.x, this.start.y, this.stop.x, this.stop.y);
        return result;
    }
    ...
    public Scale(factor: Vec2, keepCenter: boolean = true): Rect {
        let ow = this.stop.x - this.start.x;
        let oh = this.stop.y - this.start.y;
        let w = ow;
        let h = oh;
        w *= factor.x;
        h *= factor.y;
    ...
    public Area(): number {
        let x = this.stop.x - this.start.x;
        let y = this.stop.y - this.start.y;
        return x * y;
    }
