...
import type { ICloneable, ICtor } from "../../@types/index.js";
...
export class GraphNode<T> implements ICloneable {
  ...
  public clone(): this {
    ...
    result._id = this._id;
    result._data = isNullOrUndefined(this._data)
      ? this._data
      : clone(this._data);
  ...
  public connect(
    node: GraphNode<T>,
    cost?: number | null,
    edgeType?: string | [string, string] | null,
  ) {
...
export class Graph<T> {
  ...
  private _nodeIndexer: (node: GraphNode<T>) => number | string = (
    node: GraphNode<T>,
  ) => node.id;
  private _lastId: -1;
  ...
  constructor(
    id?: string,
    nodeIndexer?: (node: GraphNode<T>) => number | string,
    idGenerator?: () => number,
  ) {
  ...
  public get(id: number): GraphNode<T> | undefined {
    once(() => {
      console.warn(
        "Function Graph::get is deprecated please use Graph::findById instead. get is a reserved word.",
      );
    });
            ...
            .connect(
              ...
              isNullOrUndefined(costs)
                ...
                : isArray(costs)
                  ? (costs as number[])[i]
                  : (costs as number),
              isNullOrUndefined(edgeTypes) ? undefined : edgeTypes[i],
            );
        }
      }
    });
  ...
  public traverse<S>(
    ...
    pathSelecorFn: (
      node: GraphNode<T>,
      prevNode: GraphNode<T> | null,
      acc: S,
      traversed: List<GraphNode<T>>,
    ) => number[] | null,
    ...
    assert(
      entryPoints.length === startValues.length,
      "There has to be exactly 1 start value per entry point",
    );
    let queue = zip(
      create(entryPoints.length, () => null as GraphNode<T> | null),
      entryPoints,
      startValues,
    );
    ...
    while (
      ...
      (stopCondition === undefined ||
        stopCondition(state![1]!, state![2]!) !== false)
    ) {
      let prev = state![0];
      let node = state![1];
      let acc = state![2];
