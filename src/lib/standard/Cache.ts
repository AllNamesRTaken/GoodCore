...
import { Util, ObjectWithFunctions } from "../Util";
import { Arr } from "../Arr";
...
export class Cache<T> {
    static DEFAULT_FIFO_SIZE: number = 100;
    ...
    public Cache(obj: Object, fnName:string, keyFn?: (...args: any[]) => string): void {
        if(keyFn === undefined) {
            keyFn = function(...args: any[]): string {
                return <string>Util.Md5(Arr.Reduce(args, (acc: string, cur: any) => acc += JSON.stringify(cur)));
            }
        }
        let proxyFn = (superFn: Function, ...args: any[]) => {
            let key = keyFn(...args);
            if(key !== null && this.Hit(key)) {
                return this.Get(key);
            }
            let result = superFn(...args);
            if(key !== null) {
                this.Add(key, result);
            }
            return result;
        ...
        Util.ProxyFn(<any>obj, fnName, proxyFn, false);
    }
    public Clear() {
        this._data.Clear();
        this._order.Clear();
