export class CalcConst {
  ...
  public static ROTATION_LOOKUP: number[][] = (function (): number[][] {
    ...
    for (let i = 0; i < 360 * CalcConst.ROTATION_DEGREE_PRECISION; i++) {
      lookup.push([
        Math.cos(i * CalcConst.RADIAN_FACTOR),
        Math.sin(i * CalcConst.RADIAN_FACTOR),
      ]);
    }
    return lookup;
  })();
...
export function rotationRad(rotation: number): number[] {
  const deg =
    (rotation * CalcConst.DEGREE_FACTOR * CalcConst.ROTATION_DEGREE_PRECISION) |
    0;
  return rotationDeg(deg / CalcConst.ROTATION_DEGREE_PRECISION);
}
export function closestRadianRotation(radian: number): number {
  let deg =
    (radian * CalcConst.DEGREE_FACTOR * CalcConst.ROTATION_DEGREE_PRECISION) |
    0;
  while (deg < 0) {
    deg += CalcConst.DEG360;
  }
